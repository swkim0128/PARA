...
## 🔜 다음 세션 착수 지점 — 배포 전 검증 게이트 (2026-09-24, **최신**)

> **이 절이 아래 모든 과거 브리핑보다 우선한다.** 상세는 [[작업별-모델-라우팅-20260923]].

**해결하려던 문제**: 오류·기능개선 작업 후 배포했는데 그 기능에 대한 테스트가 부실해 추가 수정·오류가 발생한다.

**진단** — 접수→배포 전 구간에 **사람 개입 없이 자동 발동하는 품질 게이트가 0개**였다. 훅 9개 중 품질 게이트 없음, 업무 레포 CI 없음, MR 템플릿 체크리스트 없음. `요구사항` 이라는 단어가 모든 SOP·AGENTS.md·CLAUDE.md 에서 0건이었다 — 테스트 통과 여부만 보고 **요청받은 것과 만든 것을 대조하는 단계가 없었다.**

**만든 것 (전부 커밋·푸시·배포 완료)**
- `test-plan` 스킬 — 요구사항↔구현 대조 + 시나리오 도출(정상·분기·예외) + 사용자 합의 게이트 + 테스트 구현·실행(Phase 7~9). 실사용 1회에서 기존 테스트 7건이 통과하는 코드의 결함 후보 2개를 찾았다.
- `commit-verify-gate` 훅 — 이슈 키 브랜치에서 직전 커밋 이후의 `VERIFY-PASS` 기록이 없으면 커밋 차단. 기록에 브랜치명이 있어야 인정하므로 판정 단위가 날짜가 아니라 작업이다. Claude·Codex 양쪽 배선.
...
- ✅ 로컬 분류기 완료·커밋됨. 적대적 검증으로 결함 3건(과매칭·과소매칭·복합요청) 수정, 단위 18/18 + 하네스 3종 통과. 배포·설치도 반영됐다(`정본 대조 일치`).
- ✅ **푸시 완료 (2026-09-24)**: 세 레포 모두 원격과 동기화됐다. dotfiles 는 원격에 이미 같은 심링크 수정이 있어 리베이스로 중복이 걸러졌다.
- 🟡 **프록시 초안 완료·부분 검증** — `route-proxy.py` 가 `turn/start` 만 재작성한다. 재작성 로직 7/7·unix 바인드·프록시 통과 JSON-RPC 실왕복까지 확인했다. `ws://` 대신 `unix://` 라 WebSocket 프레이밍이 없고 런타임 의존성 0 이다.
- 🔴 **다음 행동 (Codex 사용량 회복 후)**: `codex --remote unix://DIR` 로 **실제 TUI 가 붙는지**만 확인하면 끝난다. `codex exec` 엔 `--remote` 가 없어 대화형으로만 가능하고, 2026-09-23 에 사용량 한도로 막혀 여기서 멈췄다. 이 한 축이 설계 전체를 떠받친다 — 안 붙으면 `ws://` 선회(프레이밍 추가)다. 재개 절차·에러 기록 요령은 [[작업별-모델-라우팅-20260923]] 「매 입력 자동 라우팅 프록시」 절.
