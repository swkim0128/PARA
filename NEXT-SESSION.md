...
### 🧹 진입 페이지 정리 — 2026-08-09 (Project · Food · Diary)
...
- **Budge**: `예상 지출 내역`(예산+계획 혼재) → **`이번 달 예산`**(HL) / **`지출 계획`**(Ledger 고정지출·지출보드)로 분리, `이전 지출 내역` → **`실행 완료` 토글**(Ledger 완료분 — 실거래가 아니라 위시 항목의 완료분이라 이름이 오해를 불렀음). Quick Actions 동기화 블록의 버튼 3개는 `<unknown url=... alt="button"/>` 로 재현해 보존(검증 완료).
  - ⚠️ **💳 거래내역(실거래 1,911건)이 Budge 페이지에 없다.** 추가 여부는 미결 — 넣으면 페이지가 무거워진다.
