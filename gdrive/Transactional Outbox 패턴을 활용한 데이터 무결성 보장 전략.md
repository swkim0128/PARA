제공된 소스들을 바탕으로 **Dual Write(이중 쓰기) 문제**와 이를 해결하여 **데이터 유실을 방지**하는 **Transactional Outbox 패턴**에 대해 심층적으로 논의하겠습니다.  
마이크로서비스 아키텍처에서 가장 빈번하고 치명적인 문제는 로컬 데이터베이스를 업데이트하는 작업과 외부 시스템(메시지 브로커 등)으로 이벤트를 발행하는 작업을 동시에 수행해야 할 때 발생하는 **Dual Write 문제**입니다. Transactional Outbox 패턴은 이 두 작업의 \*\*원자성(Atomicity)\*\*을 보장하여 데이터 유실을 막는 핵심적인 해결책입니다.
