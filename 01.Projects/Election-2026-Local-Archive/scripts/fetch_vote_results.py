...
중앙선관위 OpenAPI `VoteXmntckInfoInqireService2/getVoteSttusInfoInqire` 호출.
시도/구시군 단위 투표 통계만 반환하며, 후보별 득표는 별도 API 또는 파일 다운로드 필요.

사용 예:
    python -m scripts.fetch_vote_results --sg-id 20260603 --sg-type 3
"""
...
ENDPOINT = f"{BASE_URL}/VoteXmntckInfoInqireService2/getVoteSttusInfoInqire"


def parse_args() -> argparse.Namespace:
    """CLI 인자를 파싱한다."""
    parser = argparse.ArgumentParser(description="투·개표 정보 수집")
    ...
    parser.add_argument("--wiw-name", default=None, help="구시군명 (선택)")
...
def fetch_page(
    ...
    """단일 페이지를 호출하고 응답 dict 를 반환한다.

    httpx 가 ServiceKey 를 자동 URL 인코딩하므로 디코딩 키를 그대로 넘긴다.
    """
    params: dict[str, Any] = {
...
def main() -> None:
    ...
    with create_client() as client:
        ...
        while True:
            payload = fetch_page(
                ...
                wiw_name=args.wiw_name,
            )
            raw_name = f"vote_results_sg{args.sg_type}_{sd_tag}_p{page_no}"
    ...
    out_path = parsed_dir / f"vote_results_sg{args.sg_type}.parquet"
