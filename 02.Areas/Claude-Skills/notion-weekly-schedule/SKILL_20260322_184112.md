...
description: "노션 '일지 및 회고' 데이터베이스에서 이번 주 일정 페이지를 찾아 조회하고, 사용자가 입력한 일정 내용을 해당 페이지의 적절한 위치에 추가합니다. 자연어 입력을 분석하여 날짜, 시간, 카테고리(WORK/LIFE)를 자동으로 판단하여 일정을 추가합니다. 주간 일정 추가, 일정 기록, 노션 다이어리 업데이트 요청 시 반드시 이 스킬을 사용하세요."
---
